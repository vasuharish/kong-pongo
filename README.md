...
1. Run Pongo
    ...
    Since we share this with the host, we first restart to make sure the
...
1. Cleanup and exit
    ...
    The test environment runs on the host, so when this Pongo container exits
    we do not want
    anything to be left behind on the host, hence we shutdown the
